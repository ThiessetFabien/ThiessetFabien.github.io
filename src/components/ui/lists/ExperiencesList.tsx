...
export const ExperiencesList: React.FC<{
  ...
  <div
    className={cn('relative z-0 border-l border-primary', cnPaddingX, cnSpaceY)}
  >
    {experiences &&
      ...
      experiences
        ...
        .map((experience, expIndex) => (
          <div
            key={`exp-${expIndex}-${experience.job}`}
            className={cn('relative', cnSmallSpaceY)}
          >
            <div
              className={cn(
            ...
            <ul className={cn(cnDescription, cnSmallSpaceY)}>
              {experience.tasks &&
                ...
                experience.tasks.map((task, taskIndex) => (
                  <li
                    key={`task-${taskIndex}-${experience.id || taskIndex}`}
                    className='flex'
                  >
                    <Dot />
              ...
              {experience.stack && experience.stack.length > 0 && (
                <li className={cn(cnSmallText, 'font-bold')}>
                  Stack:{' '}
                  {experience.stack.map((item, stackIndex) => (
                    <span
                      key={`stack-${stackIndex}-${experience.id || stackIndex}`}
                      className={cn(cnLightTextMuted, 'italic')}
                    >
                      {capitalizeFirstLetterOfEachWord(
                        formatSpecialWords(item)
                      )}
                      {stackIndex < (experience.stack ?? []).length - 1
                        ? ' · '
                        : '.'}
                    </span>
                  ))}
              ...
              {experience.skills && experience.skills.length > 0 && (
                <li className={cn(cnSmallText, 'font-bold')}>
                  Compétences:{' '}
                  {experience.skills.map((item, skillIndex) => (
                    <span
                      key={`skill-${skillIndex}-${item}`}
                      className={cn(cnLightTextMuted, 'italic')}
                    >
                      {capitalizeFirstLetterOfPhrase(formatSpecialWords(item))}
